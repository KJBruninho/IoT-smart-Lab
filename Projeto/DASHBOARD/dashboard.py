...
def load_data():
    ...
    BEGIN
        ...
        SELECT TOP 100
            ...
            d.Valor,
            d.Timestamp,
            stt.Nome_Tipo AS Tipo_Sensor,
            stt.Unidade_Medida,
            g.Nome_Group AS Nome_Grupo
        FROM [SensorData] d
        JOIN Sensor s ON d.ID_Sensor = s.ID_Sensor
        JOIN SensorType stt ON s.ID_SensorType = stt.ID_SensorType
        JOIN Station st ON s.ID_Station = st.ID_Station
        JOIN [UserGroup] g ON st.ID_Group = g.ID_Group
