...
def get_db_connection():
    try:
        ...
        conn_str = ("DRIVER={SQL Server Native Client 11.0};Data Source=(localdb)\\MSSQLLocalDB;DATABASE=DATABASE_51619;Integrated Security=True;Persist Security Info=False;Pooling=False;MultipleActiveResultSets=False;Encrypt=no;TrustServerCertificate=no;Application Name=\"SQL Server Management Studio\";Command Timeout=30")
...
async def get_sensors():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT ID_Sensor, Tipo_Sensor, Unidade_Medida FROM Sensor")
